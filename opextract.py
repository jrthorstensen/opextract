...
parser.add_argument('--production','-p',help="Write .ms file (not .ms_test).",
action='store_true')
...
class irafcurve :
    ...
    def evalfit(self,x) :  # evaluate fit for an array of x-values.
        ...
        if self.curvetype == 'spline3' : 
            ...
            for i in range(0,self.iraforder) : 
...
def fake_multispec_data(arrlist) : 
   ...
   for arr in arrlist :
       ...
       arrstostack.append(np.array([arr])) 

   return np.stack(arrstostack)  
...
crossdisp = np.array(range(0,rawdata.shape[1]))
...
for lineindex in range(0,rawdata.shape[0]):
...
for lineindex in range(0,rawdata.shape[0]):
    ...
    if norm[lineindex] < readnoise / gain :  # basically nothing there.
        ...
        apdata[lineindex] = apmodel(range(0,apdata.shape[1]))
    ...
    while worst_scatter > scattercut and len(pixinds) > 0 : 
        ...
        if worst_scatter > scattercut :   # reject bad pixels
            ...
            nrej_pixel = nrej_pixel + 1   
            if iteration == 0 :   
                corrected_specpts = corrected_specpts + 1
    ...
    cr_corrected_overall_flux = cr_corrected_overall_flux + np.sum(in_ap_data)
...
optimally_extracted = optimally_extracted * normfac
